...
usage() {
  echo "Usage: RASPBERRYPI_CROSS_COMPILE_HOME=<path to toolchain root> $(basename "$0") [-s:t:Tx:X]"
  echo "-E enable experimental hexnn ops"
  echo "-s [sub_makefiles] sub makefiles separated by white space"
  echo "-t [build_target] build target for raspberry makefile [default=all]"
  echo "-T only build tensorflow"
  echo "-x [hexagon library path] copy and hexagon libraries in the specified path"
  exit 1
}

if [[ -z "${RASPBERRYPI_CROSS_COMPILE_HOME}" ]]; then
    echo "RASPBERRYPI_CROSS_COMPILE_HOME should be set as an environment variable" 1>&2
    exit 1
fi
...
if [[ "${ONLY_MAKE_TENSORFLOW}" != "true" ]]; then
  ...
  CC_PREFIX="${CC_PREFIX}" RASPBERRYPI_CROSS_COMPILE_HOME="${RASPBERRYPI_CROSS_COMPILE_HOME}" \
tensorflow/contrib/makefile/compile_pi_cross_protobuf.sh -c
else
...
if [[ -z "${BUILD_TARGET}" ]]; then
    make -j"${JOB_COUNT}" -f tensorflow/contrib/makefile/Makefile \
         TARGET=PI RASPBERRYPI_CROSS_COMPILE_HOME="${RASPBERRYPI_CROSS_COMPILE_HOME}" CC_PREFIX="${CC_PREFIX}" \
